...
// redisRateLimitSvc Redis 滑动窗口限流器，复用 cache 的 redis client，集群场景下跨实例统一计数。
...
// NewRedisLimiter 创建 Redis 后端限流器。直接复用传入 IRedisCache 内部的 redis client，
// 不重复建连。Redis 异常时 fail-open（放行），保证限流组件不拖垮业务。
func NewRedisLimiter(redisCache cacheContract.IRedisCache, cfg ratelimit.Config) ratelimit.RateLimiter {
	svc, ok := redisCache.(*redisCacheSvc)
	if !ok || svc == nil {
		panic("cache: Redis 限流器仅支持框架内置的 redis 实现（NewCacheFactory）")
	}
	cfg = ratelimit.Normalize(cfg)
